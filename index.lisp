       ...
       <html>
         ...
         <body>
           ...
           <:backgammon id="board">
             ...
             <:backgammon-top>
               ,(let ((move (get-point-move game +white-goal+ piped)))
                  <:goal color="white"
                         action="move"
                         value=(when move (list piped move))
                         onclick=(when move "rr(this)")>
               ...
               <segment pip-action="pip"
                        point-action="move"
                        game=game from=0 to=6 pip=piped />
               <:bar color="white">
                 ,@(loop for p from 0 below white-bar
                         collect <:pip action="pip"
                                       value=+white-bar+
                                       selected=(when (and (eq turn :white)
                                                           piped
                                                           (= piped +white-bar+)
                                                           (get-moves game piped))
                                                  t)
                                       onclick="rr(this)">
                                 </:pip>)
               </:bar>
               <segment pip-action="pip"
                        point-action="move"
                        game=game from=6 to=12 pip=piped />
             </:backgammon-top>
             <:backgammon-bottom>
               <segment pip-action="pip"
                        point-action="move"
                        game=game from=12 to=18 pip=piped />
               <:bar color="black">
               ...
               <segment pip-action="pip"
                        point-action="move"
                        game=game from=18 to=24 pip=piped />
               ,(let ((move (get-point-move game +black-goal+ piped)))
                  <:goal color="black"
                         action="move"
                         value=(when move (list piped move))
                         onclick=(when move "rr(this)")>
